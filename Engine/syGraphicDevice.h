...
namespace sy::graphics
{
	...
	{
	public:
		GraphicDevice(ValidationMode validationMode = ValidationMode::Disabled);
		~GraphicDevice();
		...
		void SetConstantBuffer(ShaderStage stage, CBType type, ID3D11Buffer* buffer);
		void SetShaderResource(ShaderStage stage, UINT slot, ID3D11ShaderResourceView* const* ppShaderResourceViews);
		void BindSamplers(ShaderStage stage, UINT slot, UINT NumSamplers, ID3D11SamplerState* const* ppSamplers);
